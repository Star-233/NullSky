...
### 类名大小写绕过
- **原理**：PHP 对类名大小写不敏感，`unserialize()` 的类名查找会先统一转小写再匹配，因此 `O:7:"ctfshow"` 与 `O:7:"CTFSHOW"`、`O:7:"CtfShow"` 等完全等价
- **绕过场景**：当 `preg_match('/ctfshow/', $cs)` 通过大小写敏感的正则检测类名时，将类名改为全大写或大小写混合即可绕过 —— `preg_match` 匹配不到，但 `unserialize` 依然能正常实例化类并触发魔术方法
